...
| **Bedrock** | `amazon.titan-embed-text-v2:0` **AUTHORIZED and proven live** — it built the Titan index. `anthropic.claude-sonnet-4-5` is listed as available in-region but **every invocation fails**: the account's Anthropic use-case entitlement has not been granted, so both the Mantle and the legacy `InvokeModel` paths return `permission_error` / `not_found_error` / 404 "Model use case details have not been submitted for this account" (see `docs/evidence/langgraph-run-2026-09-02.md`). Anthropic models in-region are also inference-profile-only. **Listed ≠ usable — do not claim Claude-on-Bedrock works.** |
