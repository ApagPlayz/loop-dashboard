...
import type { usePrChat } from "./use-pr-chat";

/**
 * Private, local chat about one PR — billed through the owner's Claude Max
 * subscription (same local-CLI backend as the rest of the dashboard's AI). The
 * backend hands Claude the PR's diff plus read-only access to the local
 * checkout, so it answers from the actual code. Nothing here is posted to
 * GitHub.
 */
export default function PrChat({ chat }: { chat: ReturnType<typeof usePrChat> }) {
  const { messages, sendMessage, sending, error } = chat;
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
          Chat with Claude about this PR — reads the real code
        </p>
      </div>
        ...
        <div className="mb-3 max-h-72 space-y-3 overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-950/60 p-3">
          ...
          {sending && (
            <div className="flex items-center gap-2 text-xs text-zinc-500">
              <Loader2 className="h-3.5 w-3.5 animate-spin" /> Claude is reading the code…
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex items-end gap-2">
        <textarea
          ...
          onKeyDown={onKeyDown}
          placeholder="Ask Claude anything about this PR before you decide…"
          rows={2}
