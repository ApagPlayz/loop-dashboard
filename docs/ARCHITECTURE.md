...
| **Bedrock** | `amazon.titan-embed-text-v2:0` **AUTHORIZED/AVAILABLE**; `anthropic.claude-sonnet-4-5` likewise. Anthropic models in-region are inference-profile-only |
...
| 3 | **The GitHub token is the leaked account-wide `gho_` one.** Scopes `repo`, `workflow`, `read:org`, `gist` across the *entire* account; leaked into a local agent log 2026-08-31; never rotated. | **Real risk, and the largest blast radius in the project** — but **local only**. It is deliberately not deployed (§5.3), and `loopDashboardTaskRole` has zero policies (verified live), so a guessed password on the live site reaches fixtures, not GitHub. |
...
| 8 | **The account is driven by the root user with a long-lived access key** (§5). | **Real risk**, and the worst thing in the AWS account — but outside the app. |
