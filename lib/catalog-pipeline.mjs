...
const CATEGORY_RULES = [
  ...
  // Bare "search" and bare "server" used to sit in these two rules as
  // standalone triggers. Almost every MCP entry's text contains "server"
  // somewhere (that's literally what an MCP server is), and "search" shows
  // up incidentally in totally unrelated features ("search your files",
  // "search issues", "search pages"). That produced a steady stream of false
  // positives — e.g. the Filesystem MCP server landing in "Web search &
  // scraping" purely because its blurb says "...directory listing and
  // search", and the "Everything" test/reference server landing in "Cloud &
  // infra" purely because its name contains the word "Server". Require an
  // actual web-search phrase / a specific cloud vendor or workflow term
  // instead of the bare, generic word.
  //
  // The same bug also hid in bare "google" and bare "brave": both are brand
  // names, not search-product names, so any tool that merely mentions Google
  // (Workspace, Calendar, Drive, Docs, Ads, ...) or Brave (the browser) got
  // swept into "Web search & scraping" regardless of what it actually does —
  // e.g. Google Workspace (Gmail + Calendar) and AdsAgent (Google Ads) landed
  // here purely on the brand name. Require the actual search-product phrase
  // ("Google Search", "Brave Search") so the genuine cases still match.
  ["Web search & scraping", /\b(scrape|crawl|browse|web ?page|serp|firecrawl|tavily|brave ?search|google ?search|web ?search|search engine|internet search)\b/i],
  // Same bug, two more places. Bare "automation"/"automate" caught ANY kind of
  // automation — social-media scheduling, CI/CD, legal-document, infra-as-code
  // — not just browser/UI automation (e.g. Postiz, Terraform, and a pile of
  // generic "DevOps/Test/Security Engineer" role skills all landed here
  // purely because their blurb says "test automation" or "deployment
  // automation"). Require an actual browser/UI/device-automation phrase; the
  // existing browser/playwright/puppeteer/selenium/click/navigate alternatives
  // already cover genuine browser-automation tools that phrase automation
  // differently. Also: "browser" bare matched Cloudflare's "browser
  // rendering" API feature (one line among many DNS/CDN/security features),
  // sweeping a Cloud & infra tool in here on an incidental mention — exclude
  // that specific phrase.
  [
    "Browser & automation",
    /\b(browser(?!\s*rendering)|playwright|puppeteer|selenium|ui automation|device automation|desktop automation|mobile automation|automate (?:the |a )?browser|click|navigate)\b/i,
  ],
  ...
  ["Cloud & infra", /\b(aws|azure|gcp|cloudflare|kubernetes|docker|terraform|deploy|infra|vercel|netlify)\b/i],
